...
lib.AddAura = function(id, opts)
    ...
    if type(id) == "table" then
        local clones = id
        lastRankID = clones[tgetn(clones)]
    else
        lastRankID = id
    end
...
lib.Talent = function (a1, a2, a3, a4, a5, a6, a7, a8, a9)
    for i=1, 5 do
        local spellID = select(i, a1, a2, a3, a4, a5, a6, a7, a8, a9)
...
function f:CombatLogHandler(timestamp, eventType, hideCaster,
    srcGUID, srcName, srcFlags, srcFlags2,
    dstGUID, dstName, dstFlags, dstFlags2,
...
function lib:GetAuraDurationByUnit(unit, spellID, casterUnit, spellName)
    return lib.GetAuraDurationByUnitDirect(unit, spellID, casterUnit, spellName)
end
...
function lib:UnregisterFrame(frame)
    ...
    if not next(activeFrames) then
        f:UnregisterEvent("RAW_COMBATLOG")
        if playerClass == "ROGUE" then
...
setwatcher:SetScript("OnEvent", function()
    return setwatcher[event](setwatcher, event, arg1)
end)
